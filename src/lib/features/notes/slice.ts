...
export const createNote = createAsyncThunk(
	...
	async (data: NotesInput, { rejectWithValue }) => {
		try {
			...
			return rejectWithValue(
				(error as any).response?.data?.message || "Failed to create note",
			);
		}
	},
);
...
export const updateNote = createAsyncThunk(
	"notes/update",
	async (
		{ id, data }: { id: string; data: Partial<Note> },
		{ rejectWithValue },
	) => {
		try {
...
export const { setQuery, setSelectedNote, clearError, clearNotes } =
	notesSlice.actions;
